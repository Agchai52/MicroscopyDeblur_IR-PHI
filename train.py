...
def train(args):
    ...
    for epoch in range(pre_epoch, args.epoch):
        for iteration, batch in enumerate(train_data_loader, 1):
            ...
            loss_d_fake = pred_fake_S  # criterion_GAN(pred_fake_S, False)

            # train with real
            real_label = netD(real_S)
            pred_real_S = (label.squeeze(1) - criterion_L1(real_label, label)) / label
            loss_d_real = pred_real_S  # criterion_GAN(pred_real_S, True)
            ...
            pred_fake_S = (label.squeeze(1) - criterion_L1(fake_label, label)) / label
            loss_g_gan_bs = pred_fake_S # criterion_GAN(pred_fake_S, True)
